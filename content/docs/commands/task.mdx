...
When prompted for the agent, set `gemini` (or any of `claude-code`, `codex`, `opencode`, `copilot`). The resulting YAML carries the override:
...
- **`agent:` value rejected** — only the five supported backends are valid. *Fix:* use `claude-code`, `codex`, `opencode`, `gemini`, or `copilot` — see [Supported Agents](/docs/concepts/supported-agents).
