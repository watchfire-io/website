...
| **Default agent** | Project default backend (`claude-code`, `codex`, `opencode`, or `gemini`) | Inherited from global default |
...
- **Custom binary paths** — per-backend path override for `claude`, `codex`, `opencode`, and `gemini` when Watchfire cannot locate the binary on `PATH`
