...
| **Logs** | Past session logs per task. Displays formatted conversation transcripts (User/Assistant messages with tool call summaries) for every supported backend — Claude Code, Codex, opencode, Gemini, and Copilot — falling back to raw PTY scrollback when no transcript is available. |
