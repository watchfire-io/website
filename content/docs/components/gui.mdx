...
- **Cross-project Insights rollup card** — fleet-level KPIs, top projects, and an `<ExportPill>` for global exports, scoped by the same `7d` / `30d` / `90d` / `All` window selector used in the Project View. See the [Insights concept page](/docs/concepts/insights#cross-project-insights-rollup) for what's aggregated and how the `tasks_missing_cost` caveat surfaces partial data.
