...
{/* TODO: capture tui-globalsettings-notifications screenshot on a Beacon TUI */}

The Notifications section of `globalsettings` configures Beacon. Every control below is gated through `models.ShouldNotify` on the daemon and persisted to `defaults.notifications` in `~/.watchfire/settings.yaml`.
