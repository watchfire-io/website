...
| `watchfire definition retrofit` | `defn retrofit` | Fold completed tasks back into the definition |
...
| `watchfire task quick` | | Batch-create tasks from a bulleted list (`--ready` / `--draft` / `--stdin`) |
...
| Command | Alias | Description |
|---------|-------|-------------|
| `watchfire integrations add telegram` | | Store the bot token (keyring) and enable the bridge |
| `watchfire telegram pair` | | Mint a one-time pairing code + deep link, wait for pairing |
| `watchfire telegram status` | | Bridge state, pairing status, paired chats |

See the [Telegram Bridge](/docs/concepts/telegram) concept page and [`watchfire telegram`](/docs/commands/telegram) for details.
...
| Tab | Content |
|-----|---------|
| **Outbound** | One row per configured adapter (webhook, Slack, Discord, Telegram, GitHub auto-PR) with a status pill (`OK` / `Disabled` / `Breaker open` / `Misconfigured`) and a `t` shortcut to fire a synthetic test event. |
...
Since Torch (v10.0.0) the overlay also carries the [Telegram bridge](/docs/concepts/telegram): a Telegram summary row with paired chats as child rows, a masked write-only token step, pairing on <kbd>p</kbd> (code + deep link + live countdown), per-chat <kbd>m</kbd> / <kbd>w</kbd> mute and watch toggles, and confirm-gated revoke.
...
|-----|--------|
| `a` | Add new task |
| `A` | Quick Add — batch-create tasks, one per bullet |
...
| **Definition** | Project definition (press `e` to edit in $EDITOR, `r` to [retrofit from completed tasks](/docs/concepts/agent-modes#retrofit-definition)) |
