...
> New to the vocabulary? Every term used here — daemon, PTY, worktree, sandbox, agent backend — is defined in plain English in the [Glossary →](/glossary).
...
| **GUI** | `Watchfire.app` | Multi-window, multi-project thin client — a persistent home (mission control) window plus one window per project, and an optional always-on-top mini-monitor | Electron |
...
| **Registry** | Process-wide `Register`/`Get`/`List` keyed by name (`claude-code`, `codex`, `opencode`, `gemini`, `copilot`, `cursor`) |
| **Shipped backends** | Claude Code, OpenAI Codex, opencode, Gemini CLI, GitHub Copilot CLI, Cursor Agent |
...
| **System prompt delivery** | Watchfire composes one canonical prompt; each backend's `InstallSystemPrompt` delivers it (CLI flag for Claude Code, `AGENTS.md` for Codex/opencode/Copilot/Cursor, `system.md` for Gemini) |
...
Five of the six backends get an isolated per-session home so the Watchfire system prompt never contaminates the user's real configuration, while auth and global settings continue to flow from the shared location:
...
| **Cursor Agent** | per-session home | `~/.watchfire/cursor-home/<project_id>/<session_id>/` | `AGENTS.md`, per-session state | `~/.cursor/` auth/config files symlinked in |
...
<Mermaid chart={`
graph TD
    SB["Sandbox (platform-specific)"] --> AG["Coding Agent (Claude Code / Codex / opencode / Gemini / Copilot / Cursor)"]
...
| **GitHub Copilot CLI** | Per-session JSONL inside `<COPILOT_HOME>/session-state/` |
| **Cursor Agent** | Per-session JSONL inside `~/.watchfire/cursor-home/<project_id>/<session_id>/` |
...
├── copilot-home/<session>/        # Per-session COPILOT_HOME (Copilot)
└── cursor-home/<project_id>/<session_id>/   # Per-session home (Cursor Agent)
```
...
│   ├── tasks/          # Task YAML files + per-task metrics sidecars
│   │   ├── 0001.yaml
│   │   ├── 0001.metrics.yaml
│   │   ├── 0002.yaml
│   │   └── ...
