...
| `/run <n>` / `/run all` | Run one task / drain every ready task. Refuses if an agent is already running — never queues, never replaces |
| `/wildfire` / `/wildfire off` | Start or stop the autonomous [Wildfire](/docs/concepts/agent-modes#wildfire) loop. While watching, wildfire relays a milestone feed instead of a raw stream |
| `/generate` / `/plan` | Generate the project definition / generate tasks from it, refusal-gated like `/run` |
| `/new` | Start a fresh chat session, clearing the conversation context |
