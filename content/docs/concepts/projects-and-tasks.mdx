...
`default_agent` is a first-class project setting. Set it when you want a project to prefer one backend by default, such as `codex`, `claude-code`, `opencode`, `gemini`, `copilot`, or `cursor`.
...
Want to try the schema in your browser? Open the [playground](/playground).
...
| `failure_reason` | string | Agent-reported failure (set by the agent when `success: false`) |
| `merge_failure_reason` | string | Auto-merge failure populated by the daemon when the silent merge can't land — e.g. dirty default branch, merge conflict, post-merge hook failure. The task's own work still landed on its `watchfire/<n>` branch; only the merge step failed |
