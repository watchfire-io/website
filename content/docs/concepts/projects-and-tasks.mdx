...
| `merge_failure_reason` | string | Auto-merge failure populated by the daemon when the silent merge can't land — e.g. dirty default branch, merge conflict, post-merge hook failure (Flare). The task's own work still landed on its `watchfire/<n>` branch; only the merge step failed |
