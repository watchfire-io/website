---
title: Overview
description: Watchfire is a remote control for AI coding agents. Orchestrate Claude Code, Codex, opencode, and Gemini CLI sessions with task files, git worktree isolation, and sandboxed execution.
---

# Watchfire

Watchfire orchestrates coding agent sessions based on task files. It supports **Claude Code**, **OpenAI Codex**, **opencode**, and **Gemini CLI**, and manages multiple projects in parallel by spawning agents in sandboxed PTYs with git worktree isolation. A daemon handles all orchestration while thin clients (CLI/TUI and GUI) connect over gRPC.
