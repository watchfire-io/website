...
const OpencodeLogo = () => (
  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className="h-7 w-7">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mx-auto mt-14 flex max-w-5xl flex-wrap justify-center gap-5">
          {agents.map((agent) => (
            <Link
              key={agent.tag}
              href={`/agents/${agent.tag}`}
              data-stagger
              className="card-hover group relative block w-full overflow-hidden rounded-2xl border border-zinc-200 bg-white/70 p-6 backdrop-blur-sm dark:border-zinc-800 dark:bg-zinc-900/60 sm:w-[calc(50%-0.625rem)] md:w-72"
              style={{
            ...
            >
              ...
              <div className="relative flex items-center gap-4">
                <div
                  className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl border transition-all duration-300 group-hover:scale-105"
                  style={{
                    color: agent.accent,
                    borderColor: `rgba(${agent.accentRgb}, 0.4)`,
                    background: `rgba(${agent.accentRgb}, 0.12)`,
                    boxShadow: `0 0 24px rgba(${agent.accentRgb}, 0.18)`,
                  }}
                >
                  <agent.Logo />
                </div>
